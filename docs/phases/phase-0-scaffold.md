...
- [ ] Запушить любое изменение текста, дождаться CI, поставить новый APK поверх без удаления старого. Результат: …
  Подсказка: вторая сборка уже есть — [run #2](https://github.com/merenkoff/syto/actions/runs/35968937505),
  артефакт `syto-debug-4352f4d` (тот же код, другой хэш). Ставить поверх `syto-debug-72ee01a`.
