...
Статус: 🔄 (код написан, ждём первого зелёного CI)
...
Статус: 🔄 (workflow написан, ждём первого прогона)
...
- Первый прогон: см. ниже «Заметки и находки», строка «CI run #…».
...
- CI run #…: заполняется после первого прогона.
