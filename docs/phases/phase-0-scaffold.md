...
Статус: ✅ · Начата: 2026-09-24 · Завершена: 2026-09-24
...
- [x] Открыть workflow run на GitHub → скачать артефакт → в архиве есть APK. Результат: артефакт `syto-debug-af6343c`
  скачан через `gh run download`, внутри один APK 29,7 МБ (2026-09-24).
...
Статус: ✅ (2026-09-24, чек-лист пройден по Wi‑Fi adb)
...
- [x] Установить APK, запустить, увидеть версию и хэш. Результат: локальная сборка `a6c1c43`, `Build = local`,
  поставлена через `adb install -r` по Wi‑Fi, экран показывает версию 0.1.0 (1), хэш, Android 16 (API 36), realme RMX3999.
- [x] Запушить любое изменение текста, дождаться CI, поставить новый APK поверх без удаления старого. Результат:
  CI [run #4](https://github.com/merenkoff/syto/actions/runs/35973580657) (коммит `af6343c`) поставлен поверх локальной
  сборки без удаления, `Success`, на экране `Commit af6343c`, `Build 4`. `firstInstallTime` сохранился — это обновление,
  а не переустановка. Подпись локальной и CI-сборки совпадает (D-009 работает).
- [x] Переписать с экрана строки «Android» и «Device» в `DECISIONS.md` (гейт G0). Результат: `Android 16 (API 36)`,
  `realme RMX3999`; из `getprop`: прошивка `RMX3999_16.0.5.1210(EX01)`, realme UI `V16.0.0`. Записано в D-006 и стенд фазы 1.

**DoD.** Все пункты чек-листа ✅. Модель и Android-версия устройства записаны в `DECISIONS.md`.

**Результат.**

- 2026-09-24, Mac. USB-кабель не дал соединения (Mac не видел телефон даже в `system_profiler`, кабель, похоже,
  без линии данных). Перешли на **отладку по Wi‑Fi**: на телефоне «Отладка по Wi‑Fi» → «Подключить устройство
  с помощью кода», агент делает `adb pair <ip:port> <код>`, после чего порт для `adb connect` виден через `adb mdns services`.
  Описано ниже в «Как ставить APK», путь A′.
- Скачать артефакт с Mac проще всего через `gh run download <run-id>`: `gh` стоит и залогинен, без браузера и zip вручную.
- Гейт G0 пройден: CI-сборка стоит на телефоне и показывает свой хэш и номер прогона.
...
Со стороны Mac шаг 1 короче через `gh` (стоит и залогинен):

```bash
gh run list --workflow "Build debug APK" --limit 5
gh run download <run-id> --dir ~/Downloads/syto
adb install -r ~/Downloads/syto/syto-debug-<sha7>/syto-debug-<sha7>.apk
```

### A′. Wi‑Fi вместо кабеля (проверено 2026-09-24)

Если кабель не даёт данных, adb работает по Wi‑Fi (Android 11+). Телефон и Mac в одной сети.

1. Телефон: Параметры разработчика → **Отладка по Wi‑Fi** → включить → **Подключить устройство с помощью кода**.
   На экране — шестизначный код и `ip:port` для спаривания.
2. Mac, один раз на пару телефон↔Mac:
   ```bash
   adb pair <ip:port-спаривания> <код>
   adb mdns services        # покажет ip:port сервиса _adb-tls-connect
   adb connect <ip:port-подключения>
   ```
3. Телефон появляется в `adb devices` дважды — по IP и по mDNS-имени. Чтобы adb не ругался «more than one device»,
   зафиксировать один: `export ANDROID_SERIAL=<ip:port-подключения>`.
4. Дальше `adb install -r` как в A. Порт подключения меняется после перезагрузки телефона или выключения отладки по Wi‑Fi:
   тогда снова `adb mdns services` и `adb connect`, спаривать заново не нужно.

`adb` на Mac лежит в `~/Library/Android/sdk/platform-tools/adb`, в `PATH` его нет.
...
- На телефоне стоит Viber с включённым «Определителем номера»: при входящем звонке realme UI показывает его баннер.
  Значит, в фазе 2 роль `ROLE_CALL_SCREENING` может быть уже занята Viber'ом, и при запросе роли Syto будет её отбирать.
  Учесть в стенде фазы 2.
...
- CI run #1 (2026-09-24): зелёный, ~2 мин на холодном кэше. Ни одна из версий из таблицы 0.1 не потребовала правок.
  В логе нет ни одного предупреждения от AGP, Kotlin или Compose-плагина. Конфигурация (скачивание зависимостей)
  заняла ~57 с из 1 мин 55 с сборки. `gradle/actions` подтвердил подлинность `gradle-wrapper.jar`
  (sha256 `497c8c2a…a9c7` есть в его списке известных wrapper'ов). Debug-APK — 29,7 МБ (zip артефакта 10,4 МБ).
- На раннере платформы лежат как `android-37.0`, `android-37.1`, `android-37.2`; `compileSdk = 37` AGP 9.4 резолвит
  в `android-37.0` без докачки. Build-Tools 36.0.0 и 37.0.0 тоже на месте.
- CI run #2 (2026-09-24, коммит `4352f4d`, только docs): зелёный, артефакт `syto-debug-4352f4d`. Конвейер воспроизводим.
- Кэш Gradle в `gradle/actions/setup-gradle` на не-default ветках только читается, поэтому каждый прогон на ветке
  холодный. Если станет мешать — `cache-read-only: false` в шаге setup-gradle (пишет кэш и с веток). Пока не трогаем.
